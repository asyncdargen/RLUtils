...
import ru.redline.mod.gui.element.GuiPasswordField;
...
public class GuiAltManager extends GuiScreen {
    ...
    private final RenderProvider render = RLUtils.INSTANCE.getRender();
    ...
    protected void actionPerformed(GuiButton button) throws IOException {
        ...
        switch (button.id) {
            ...
            case 2: RLUtils.INSTANCE.getPlayerProvider().getAltManager().restoreDefault();
        }
    }
    ...
    private void applySession() {
        ...
        CompletableFuture.runAsync(() -> {
            enabled = false;
            try {
                RLUtils.INSTANCE.getPlayerProvider().getAltManager().login(login, password);
            } catch (Exception e) {
                RLUtils.INSTANCE.getLogger().info("Error while login : " + e.getMessage());
            }
            enabled = true;
        });
        userField.setText("");
