...
public class GuiPasswordField {
    ...
    private RenderProvider render = RLUtils.INSTANCE.getRender();
    ...
    public GuiPasswordField( int x, int y, int par5Width, int par6Height) {
    ...
    public void drawTextBox() {
        if (this.getVisible()) {
            if (this.getEnableBackgroundDrawing()) {
                render.drawRect(this.x - 1, this.y - 1, this.width + 2, this.height + 2, -6250336);
                render.drawRect(this.x, this.y, this.width, this.height, -16777216);
            }

            int i = this.isEnabled ? this.enabledColor : this.disabledColor;
            int j = this.cursorPosition - this.lineScrollOffset;
            int k = this.selectionEnd - this.lineScrollOffset;
            String s = Strings.repeat('*', text.length());
            ...
            if (flag1) {
                if (flag2) {
                    Gui.drawRect(k1 - 1, i1 - 1, k1 + 2, i1 + 2 + this.fontRenderer.FONT_HEIGHT, -3092272);
                } else {
